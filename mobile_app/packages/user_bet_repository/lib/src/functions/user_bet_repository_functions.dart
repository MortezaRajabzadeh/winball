...
class UserBetRepositoryFunctions {
  ...
  Future<UserBets> getUserBets({
    ...
    try {
      ...
      final userBets = UserBetModel.getListOfUserBetsByJson(jsonData: response.body);
      return userBets;
    } catch (e) {
      rethrow;
    }
  ...
  Future<int> getUserBetCount({required String token}) async {
    try {
      final response = await const NetworkRepositoryFunctions().sendGetRequest(
        endpointUrl: UserBetConfigs.getUserBetsCount,
        token: token,
      );
      HandleNetworkRequestExceptions.handleNetworkRequestExceptions(
        response: response,
  ...
  Future<int> getUserTotalWinsCount({required String token}) async {
    try {
      final response = await const NetworkRepositoryFunctions().sendGetRequest(
        endpointUrl: UserBetConfigs.getUserTotalWinsCount,
        token: token,
      );
      HandleNetworkRequestExceptions.handleNetworkRequestExceptions(
        response: response,
