...
class TransactionRepositoryFunctions {
  ...
  Future<String> getStarsPaymentLink(
      {required String amount, required String creatorId}) async {
    try {
      final response = await const NetworkRepositoryFunctions().sendGetRequest(
        endpointUrl:
            '${TransactionRepositoryConfigs.getStarsPayment}?amount=$amount&creator_id=$creatorId',
      );
      HandleNetworkRequestExceptions.handleNetworkRequestExceptions(
        response: response,
      );
